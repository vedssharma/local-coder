...
def build_prompt_with_context(prompt: str, file_contents: dict[str, str]) -> str:
    ...
    context = "\n\n".join(context_parts)
    full_prompt = f"You are an expert and helpful coding assistant that is good at analyzing code files and can explain what the code in a file does. The user has provided the following files for context:\n\n{context}\n\nUser: {prompt}\n\nAssistant:"
    return full_prompt
...
def build_edit_prompt(prompt: str, file_contents: dict[str, str]) -> str:
    ...
    prompt += "2. Figure out the best solution to implement\n"
    prompt += "3. Generate the complete modified content for each file that needs changes\n"
    prompt += "4. Output in this format:\n\n"
